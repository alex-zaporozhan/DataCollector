...
public class UserHelper implements Helper {
  ...
  public int readRecords(InputStream inputStream) {
    try (BufferedReader reader = new BufferedReader(new InputStreamReader(inputStream))) {
      ...
      while (reader.readLine() != null) {
        String[] values = reader.readLine().split(CSV_DELIMITER);
  ...
  public boolean writeRecords(OutputStream outputStream) {
    ...
    try {
      ...
      for (User user : users) {
        if (first) {
          outputStream.write(join(CSV_DELIMITER,user.toCsvRecord().keySet()).getBytes());
          outputStream.write("\n".getBytes());
        }
        first = false;
        outputStream.write(join(CSV_DELIMITER,user.toCsvRecord().values()).getBytes());
        outputStream.write("\n".getBytes());
      }
      return true;
    } catch (IOException e) {
